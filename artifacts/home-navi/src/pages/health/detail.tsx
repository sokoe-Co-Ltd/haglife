...
import { ChevronLeft, Stethoscope, Activity, Utensils, Baby, Weight } from "lucide-react";
      ...
      <div className="space-y-6 max-w-5xl mx-auto h-[calc(100vh-6rem)] flex flex-col">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 min-h-0">
             <Skeleton className="h-full w-full col-span-1 rounded-xl" />
             <Skeleton className="h-full w-full col-span-2 rounded-xl" />
          </div>
        ) : !resident || !summary ? (
           <div className="text-center py-12">見つかりませんでした</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 min-h-0">
            {/* Left/Middle Pane: Resident Info */}
                ...
                <CardContent className="p-6">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground mt-1">
                        {resident.gender}・要介護: {resident.careLevel}
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm">
                     <div className="flex justify-between py-1 border-b border-primary/10">
                       ...
                       <span className="font-medium">{resident.clinic1 || "未登録"}</span>
                     </div>
                     <div className="flex justify-between py-1 border-b border-primary/10">
                       <span className="text-muted-foreground">クリニック2</span>
                       <span className="font-medium">{resident.clinic2 || "未登録"}</span>
                     </div>
                  </div>
                </CardContent>
              </Card>
